...
class Trainer:
    def __init__(self, options):
        ...
        self.models["depth"] = networks.MaDepthDecoder()
        ...
        val_dataset = self.dataset(
            ...
            frames_to_load, 4, is_train=False, is_val=False, img_ext=img_ext)
        self.val_loader = DataLoader(
            val_dataset, self.opt.batch_size, True,
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=True)
        self.val_iter = iter(self.val_loader)

        test_filenames = readlines(os.path.join(os.path.dirname(__file__), "splits", 'cityscapes', "test_files.txt"))
        test_dataset = datasets.CityscapesEvalDataset(
            self.opt.data_path, test_filenames, self.opt.height, self.opt.width,
            [0, -1], 4, is_train=False, img_ext=img_ext)
        self.test_loader = DataLoader(
            test_dataset, 4, shuffle=False,
            num_workers=4, pin_memory=True, drop_last=False)
        self.metrics_names = ["abs_rel", "sq_rel", "rmse", "rmse_log", "a1", "a2", "a3"]
        self.best_metrics = np.array([np.inf, np.inf, np.inf, np.inf, -np.inf, -np.inf, -np.inf])  # 初始化历史最佳成绩
        for metric_name in self.metrics_names:  # 创建保存目录
            os.makedirs(os.path.join(self.log_path, metric_name), exist_ok=True)
    ...
    def process_batch(self, inputs, is_train=False):
        ...
        outputs.update(self.models["depth"](features, inputs["edges"]))
    ...
    def test_eigen(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.test_loader):
                ...
                output, lowest_cost, costvol = self.models["encoder"](input_color, lookup_frames,
                                                                      ...
                                                                      K,
                                                                      invK,
                                                                      self.min_depth_tracker, self.max_depth_tracker)
                output = self.models["depth"](output,edges)
        ...
        gt_depths = '/data/coding/gt_depths/'
        ...
        for i in range(pred_disps.shape[0]):
            ...
            errors.append(compute_errors2(gt_depth, pred_depth))
        ...
        is_better = [mean_errors[i] < self.best_metrics[i] if i < 4 else mean_errors[i] > self.best_metrics[i] for i in
                     range(7)]
        for i, better in enumerate(is_better):
            if better:
                self.best_metrics[i] = mean_errors[i]
                metric_name = self.metrics_names[i]
                save_path = os.path.join(self.log_path, metric_name, "best_model")
                self.save_model2(save_path)
                save_npl = save_path+'/best.npy'
                np.save(save_npl, pred_disps)
                print(f"New best model saved for {metric_name} at epoch {self.epoch} with value {mean_errors[i]}")
        self.set_train()
    ...
    def compute_depth_losses(self, inputs, outputs, losses):
        ...
        for i, metric in enumerate(self.depth_metric_names):
            ...
            f.write(str(depth_errors[i].cpu().item()) + ',')
    ...
    def save_model2(self,save_folder):
        """Save model weights to disk
        """
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)

        epoch_num = save_folder + '/epoches.txt'
        with open(epoch_num, 'a') as file:
