...
from networks.depth_decoder_mamba import sobel_filter
...
class MonoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if do_color_aug:
            ...
            color_aug = transforms.Lambda(lambda x: x)
        self.preprocess(inputs, color_aug)

        inputs["edges"] = sobel_filter(inputs[("color", 0, 0)])
        for i in self.frame_idxs:
        ...
        if self.is_val:
            depth_path = self.data_path + '/val_depth/' + folder + '_' + str(frame_index) + '_' + side + '.npy'
