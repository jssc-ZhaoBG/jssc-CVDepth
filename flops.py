...
def prepare_decoder_input_resnet(resolution):
    ...
    x1 = torch.FloatTensor(1, 64, resolution[0]//2, resolution[1]//2)
    x2 = torch.FloatTensor(1, 64, resolution[0]//4, resolution[1]//4)
    x3 = torch.FloatTensor(1, 128, resolution[0]//8, resolution[1]//8)
    x4 = torch.FloatTensor(1, 256, resolution[0]//16, resolution[1]//16)
    x5 = torch.FloatTensor(1, 512, resolution[0]//32, resolution[1]//32)

    return  dict(input_features=[x1, x2, x3,x4,x5])

def prepare_decoder_input_mam(resolution):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    x1 = torch.FloatTensor(1, 64, resolution[0]//2, resolution[1]//2).to(device)
    x2 = torch.FloatTensor(1, 64, resolution[0]//4, resolution[1]//4).to(device)
    x3 = torch.FloatTensor(1, 128, resolution[0]//8, resolution[1]//8).to(device)
    x4 = torch.FloatTensor(1, 256, resolution[0]//16, resolution[1]//16).to(device)
    x5 = torch.FloatTensor(1, 512, resolution[0]//32, resolution[1]//32).to(device)
    edges = torch.FloatTensor(1, 3, *resolution).to(device)
    return  dict(input_features=[x1, x2, x3,x4,x5],edges = edges)
...
def prepare_decoder_input_cmt_l3(resolution):
    ...
    x2 = torch.FloatTensor(1, 64, resolution[0]//4, resolution[1]//4)
    x3 = torch.FloatTensor(1, 128, resolution[0]//8, resolution[1]//8)
    x4 = torch.FloatTensor(1, 184, resolution[0]//16, resolution[1]//16)
    x5 = torch.FloatTensor(1, 368, resolution[0]//32, resolution[1]//32)
    return  dict(input_features=[x1, x2, x3,x4,x5])
